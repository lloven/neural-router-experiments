...
#SBATCH --output=/scratch/project_2018951/neural-router/logs/qoe-calfrac-050-tail-%j.out
#SBATCH --error=/scratch/project_2018951/neural-router/logs/qoe-calfrac-050-tail-%j.err
# =============================================================================
# Tail-job for calfrac frac_0.50: 9 missing cells from 6628947 walltime kill.
# Missing seeds {0, 789} across hom/rr and {0, 456, 789} for qoe_optimised.
# Runs --seeds 0,456,789; post-job merge dedups by tuple.
# =============================================================================
...
echo "=== qoe-calfrac-050-tail $SLURM_JOB_ID on $(hostname), port $PORT ==="
nvidia-smi -L
date
$OLLAMA serve > $NR_ROOT/logs/ollama-qoe-calfrac-050-tail-$SLURM_JOB_ID.log 2>&1 &
OLLAMA_PID=$!
...
for i in $(seq 1 30); do
    ...
    if [ "$i" -eq 30 ]; then
        ...
        tail -20 $NR_ROOT/logs/ollama-qoe-calfrac-050-tail-$SLURM_JOB_ID.log >&2
        exit 1
    fi
    sleep 1
done
...
rm -rf "$FULL_OUT/frac_0.50_tail"
mkdir -p "$FULL_OUT/frac_0.50_tail"

echo "=== frac_0.50 tail (seeds 0,456,789) ==="
python scripts/run_qoe.py \
    ...
    --output-dir "$FULL_OUT/frac_0.50_tail"

CSV="$FULL_OUT/frac_0.50_tail/qoe_D1.csv"
if [ -s "$CSV" ] && [ "$(wc -l < "$CSV")" -gt 1 ]; then
    echo "PASS frac_0.50_tail: $(wc -l < "$CSV") lines"
else
    echo "FAIL frac_0.50_tail: $CSV empty/missing"
    exit 1
fi

echo "=== DONE qoe-calfrac-050-tail ==="
date
